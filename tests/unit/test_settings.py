...
class TestSettings:
    def test_settings_with_valid_env_vars(self, mock_env_vars):
        ...
        assert str(settings.youtube_playlist_url) == mock_env_vars["YOUTUBE_PLAYLIST_URL"]
    ...
    def test_settings_with_missing_required_url(self):
        ...
        with patch.dict("os.environ", {"YOUTUBE_PLAYLIST_URL": ""}, clear=True):
            with pytest.raises(ValidationError) as exc_info:
                Settings()
            
            errors = exc_info.value.errors()
            assert any(error["loc"] == ("youtube_playlist_url",) for error in errors)

    def test_settings_with_invalid_youtube_url(self):
        """Test that non-playlist YouTube URL raises validation error."""
        with patch.dict("os.environ", {"YOUTUBE_PLAYLIST_URL": "https://www.youtube.com/watch?v=123"}, clear=True):
            with pytest.raises(ValidationError) as exc_info:
                Settings()
    ...
    def test_ensure_directories_creates_paths(self, temp_dir, mock_env_vars):
    ...
    def test_default_values(self):
        ...
        minimal_env = {"YOUTUBE_PLAYLIST_URL": "https://www.youtube.com/playlist?list=PLtest"}
